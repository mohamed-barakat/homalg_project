...
|__/                   |\
""",
            init_string := "using Singular; import Nemo; import AbstractAlgebra",
...
function Singular.vector(R::PolyRing{T}, a::Array)::svector where T <:AbstractAlgebra.RingElem
   Singular.vector(R, a...)
end

function Singular.Module(R::PolyRing{T}, vecs::Array{svector{spoly{T}},1})::Singular.smodule where T <:AbstractAlgebra.RingElem
   Singular.Module(R, vecs...)
end

function Singular.Matrix(R::PolyRing{T}, r::Int, c::Int, a::Array{spoly{T},1})::smatrix where T<:AbstractAlgebra.RingElem
    Singular.transpose(Singular.Matrix(Singular.Module(R, [Singular.vector(R, a[c*(i-1)+1:c*i]) for i in 1:r])))
end

function Singular.Matrix(R::PolyRing, r::Int, c::Int, a::Array)::smatrix
    Singular.Matrix(R, r, c, [R(e) for e in a])
end

function Singular.Module(R::PolyRing{T}, a::Array{spoly{T},2})::Singular.smodule where T <:AbstractAlgebra.RingElem
...
function Singular.check_parent(I::smodule{T}, J::smodule{T}) where T <: AbstractAlgebra.RingElem
   base_ring(I) != base_ring(J) && error("Incompatible modules")
end

function Singular.reduce(M::smodule, G::smodule)
...
function Singular.dimension(I::smodule{S}) where S <: Union{spoly{T}, spoly{Singular.n_unknown{U}}} where {T <: Singular.FieldElem, U <: Nemo.FieldElem}
   I.isGB == false && error("I needs to be a Gröbner basis.")
   R = base_ring(I)
   return Int(Singular.libSingular.scDimInt(I.ptr, R.ptr))
end

function Singular.dimension(I::smodule{S}) where S <: Union{spoly{T}, spoly{Singular.n_unknown{U}}} where {T <: Singular.n_Z, U <: Nemo.Integer}
   I.isGB == false && error("I needs to be a Gröbner basis.")
   R = base_ring(I)
   return Int(Singular.libSingular.scDimInt(I.ptr, R.ptr))
end
...
function AbstractAlgebra.matrix(R::PolyRing{T}, a::Array{spoly{T},2})::AbstractAlgebra.Generic.MatSpaceElem where T <:AbstractAlgebra.RingElem
...
function IdentityMatrixForHomalg(R, r)
    ...
    AbstractAlgebra.matrix(Singular.Module(R, id))
end
...
#function Singular.Matrix(R::PolyRing{T}, a::Array{spoly{T},2})::smatrix{spoly{T}} where T <:AbstractAlgebra.RingElem
#    Singular.Matrix(R::PolyRing{T}, size(a)[1], size(a)[2], reshape(a,:))
#end

function isone(r::spoly{T})::Bool where T r == one(r) end
function isone(M::smatrix)::Bool nrows(M) == ncols(M) && iszero(M-IdentityMatrixForHomalg(base_ring(M),nrows(M))) end

Determinant = Singular.det

function UnionOfRows(Ms::smatrix...)::smatrix
    list = [[M[i] for i in 1:ngens(M)] for M in [Singular.Module(M) for M in Ms]]
    list = vcat(list...)
    Singular.Matrix(Singular.Module(base_ring(Ms[1]), list))
end

function UnionOfColumns(Ms::smatrix...)::smatrix
...
InstallGlobalFunction( HomalgRingOfIntegersInOscar,
  function( arg )
    ...
    if nargs > 0 and IsString( arg[1] ) then
        ...
        if nargs > 1 and IsInt( arg[1] ) then
            d := arg[1];
            arg := arg{[ 2 .. nargs - 1 ]};
        fi;
        
        r := CallFuncList( HomalgRingOfIntegersInOscar, Concatenation( [ c ], arg ) );
        
        if not IsZero( c ) then
            ZZ := Concatenation( "Singular.FiniteField(", String( c ), ", ", String( d ), ", \"", Concatenation( param ), "\")" );
            ZZ := Concatenation( [ ZZ ], [ [ "" ] ], [ [ Concatenation( ", ", Concatenation( param ) ) ] ], [ IsPrincipalIdealRing ], arg );
            ZZ := CallFuncList( RingForHomalgInOscar, ZZ );
        fi;
        
        R := [ "Singular.PolynomialRing(", ZZ, ", [\"dummy_variable\"])" ];
        
    else
        
        if not IsZero( c ) then
            ZZ := Concatenation( "Singular.FiniteField(", String( c ), ", 1, \"Zc_1\")" );
            ZZ := Concatenation( [ ZZ ], [ [ "" ] ], [ [ ", Zc_1" ] ], [ IsPrincipalIdealRing ], arg );
            ZZ := CallFuncList( RingForHomalgInOscar, ZZ );
        fi;
  ...
  function( arg )
    ...
    if nargs > 0 and IsString( arg[1] ) then
        ...
        if param = [ ] then
            ...
            Error( "rational function field not implemented yet" );
        fi;
...
InstallMethod( LoadHomalgMatrixFromFile,
  ...
  function( filename, r, c, R )
    ...
    #read the file with GAP and parse it for better Oscar reading:
    ...
    fname := Concatenation( filename, "-oscar" );
    ...
    if mode = "ListList" then
        ...
        command := [
                    ...
                    M, "=transpose(", M, "); ",	## matrices are saved transposed in Oscar
                    v, "s=\"\""
                    ];
